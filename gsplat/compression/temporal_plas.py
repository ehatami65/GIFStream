...
def reorder_plas(
    ...
    improvement_break,
    pbar,
    static_target: Optional[Tensor] = None,
):
...
def sort_with_static_target(
    ...
    """
    Sorts a set of parameters ('params') to match a static 'target_grid'
    using the full iterative refinement loop of PLAS for maximum quality.
    """
    if seed is not None:
        torch.manual_seed(seed)
        np.random.seed(seed)
    ...
    H, W = params.shape[1:]
    assert H == W, "Only square grids are supported."

    # Initialize a grid that tracks the original indices
    grid_indices = (
        torch.arange(0, H * W, dtype=torch.int32, device=params.device)
        .reshape(H, W)
        .unsqueeze(0)
    ...
    # --- Use the full iterative refinement loop ---
    radius_f = max(H, W) / 2 - 1
    # Note: Ensure radius_seq is imported from your temporal_plas file
    radii = list(radius_seq(max_radius=radius_f, min_radius=min_blur_radius, radius_update=0.95))
    
    pbar = tqdm(radii, desc="Sorting against static target") if verbose else None
    ...
    with torch.inference_mode():
        for radius in radii:
            ...
            # Create a blurred version of our STATIC target for this iteration
            blurred_static_target = low_pass_filter(
                target_grid, filter_size_x, filter_size_y, "circular", "reflect"
            )
            
            # Call our modified reorder_plas, passing the blurred static target
            params, grid_indices, num_reorders = reorder_plas(
                params,
                grid_indices,
    ...
    duration = time.time() - start_time
    if verbose:
        print(f"\nSorted against static target in {duration:.3f} seconds with {total_num_reorders} reorders.")
